...
extern u8 cam;
int colorlist[9] = {4,6,8,5,7,3,2,1,9};
u8 area= 0x7f;
...
void o_to_c_beta(void){
  int adjustment[4] = {0, 0, 0, 0};
	...
	CON_direction = 1;
	CON_grayrequest = 1;
	Control_Set_PID_para(1, 0.16, 0.0, 0.2);
	Control_Set_PID(1, - 0.2);
	Control_Reset_PID();
	 Control_PID_Begin();
	  delay_ms(1000);
	  delay_ms(1000);
...
void area_21_circle_status0_beta(void){
	//如果有旗
			...
			Control_Set_PID(1, 0.3); 
			 Control_Set_PID(3, 0.3); 
			Control_PID_Begin();
			delay_ms(500);
			while(1){
...
void area_fetchline(){
			...
			if(((colorlist[(area - 1) / 3] == 7 - left * 3 || colorlist[(area - 1) / 3] == 8 - left * 3 )&& circle_status == 0 )|| ((colorlist[(area - 1) / 3] == 5 - left || colorlist[(area - 1) / 3] == 6 - left ) && circle_status ==1)){
			 ...
			 if(colorlist[(area - 1) / 3] == 6 || colorlist[(area - 1) / 3] == 4 || colorlist[(area - 1) / 3] == 2  || colorlist[(area - 1) / 3] == 8){
				...
				else {Control_Reset_PID(); Control_Set_PID(0, 1.5); }
			  detect_distance();
				Control_Stop();
				#ifdef ONSTM
				...
				Control_Reset_PID();
				Control_Set_PID(0, 0.0);
				if(colorlist[(area - 1) / 3] == -2) Control_Set_PID(2, 1.5);
				else {
