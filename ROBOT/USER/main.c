...
extern u8 cam;
int colorlist[9] = {0,1,0,0,2,0,0,0,0};
u8 area= 14;
...
void area_21_circle_status0(void){
	//如果有旗
			if(colorlist[area/3] == 5 - left * 2 || colorlist[area/3] == 6 - left * 2){
				o_to_b();
				//发送抓旗命令
				while(1){
					//寻求蓝牙停止信息
			...
			circle_status = 1;
			//如果在第二轮取旗在最后一条有旗子
			if(colorlist[area] == 7 - left * 2 || colorlist[area] == 8 - left * 2){
				a_to_b();
				//发送抓旗子命令
				while(1){
					//抓旗子结束信息
...
void area_atpoint_status1(void){
	    Control_PID_Begin();
	    while(colorlist[area/3] == 0){}
			if(colorlist[area%3] == 7 - left * 2 || colorlist[area%3] == 8 - left * 2){
				gray_request = 0;
				direction = 0;
				iscross = Control_Cal_Gray(gray_request,direction);
				if(!iscross){
...
void area_fetchline(){
			Control_PID_Begin();
			if(((colorlist[(area - 1) / 3] == 5 - left * 2 || colorlist[(area - 1) / 3] == 6 - left * 2 )&& circle_status == 0 )|| ((colorlist[(area - 1) / 3] == 7 - left * 2 || colorlist[(area - 1) / 3] == 8 - left * 2 ) && circle_status ==1)){
				CON_grayrequest =  0 ;
			  CON_direction = 0 ;
			  ...
			  while(1){
				...
				*/
				...
				if(colorlist[(area - 1) / 3] == -2) ;
				else {
					Control_Set_PID(2, 3.0);
				}
				CON_grayrequest = 0;
				CON_direction = 2;
				while(1){
